...
import {
    fetchCoffeeShopsAndLocations,
    type LocationItem,
} from "@/app/utilities/dataUtils";
...
const ProfilePage = () => {
    ...
    const isGoogleUser = user?.google_id || user?.is_google_user;
    ...
    // Switch to info tab if Google user tries to access password tab
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { coffeeShops, locations } =
                    await fetchCoffeeShopsAndLocations();
                setCoffeeShops(coffeeShops);
                setLocations(locations);
            } catch (error) {
                console.error(
                    "Failed to fetch coffee shops and locations:",
                    error
                );
                    ...
                    {isGoogleUser && (
                        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 text-blue-600 rounded-lg">
                            <div className="flex items-center">
                                ...
                                <p className="text-sm">
                                    You signed in with Google. To change your
                                    password, please use your Google account
                                    settings.
                                </p>
                            </div>
                        </div>
